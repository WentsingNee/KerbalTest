...
do { \
	KERBAL_TEST_CHECK_EQUAL_STATIC(kerbal::type_traits::try_test_is_nothrow_default_constructible<Type>::value, Ans::value); \
} while(0)

#define TRY_TEST_CHECK_WEAK(Type, Ans) \
do { \
	KERBAL_TEST_CHECK_STATIC( \
		kerbal::type_traits::try_test_is_nothrow_default_constructible<Type>::value == Ans::value || \
		kerbal::type_traits::try_test_is_nothrow_default_constructible<Type>::value == tribool_unspecified::value \
	); \
} while(0)
	...
	TRY_TEST_CHECK_WEAK(NothrowDefaultConstructible, tribool_true);
	TRY_TEST_CHECK_WEAK(NothrowDefaultConstructible[], tribool_false);
	TRY_TEST_CHECK_WEAK(NothrowDefaultConstructible[2], tribool_true);

	TRY_TEST_CHECK_WEAK(NonNothrowDefaultConstructible, tribool_false);
	TRY_TEST_CHECK_WEAK(NonNothrowDefaultConstructible[], tribool_false);
	TRY_TEST_CHECK_WEAK(NonNothrowDefaultConstructible[2], tribool_false);
