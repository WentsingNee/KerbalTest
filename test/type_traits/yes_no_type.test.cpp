...
KERBAL_TEST_CASE(test_yes_no_type_could_use_typedef, "test yes_no_type could use typedef")
{
	...
	KERBAL_TEST_CHECK_STATIC(could_use_typedef<FooHasTypedef>::value == true);

#if KERBAL_COMPILER_ID == KERBAL_COMPILER_ID_GNU
	KERBAL_TEST_CHECK_STATIC(could_use_typedef<FooPrivateTypedef>::value == false);
...
{
	KERBAL_TEST_CHECK_STATIC(could_use_field<FooNoField>::value == false);
	KERBAL_TEST_CHECK_STATIC(could_use_field<FooHasField>::value == true);

#if KERBAL_COMPILER_ID == KERBAL_COMPILER_ID_GNU
	KERBAL_TEST_CHECK_STATIC(could_use_field<FooPrivateField>::value == false);
#elif KERBAL_COMPILER_ID == KERBAL_COMPILER_ID_CLANG
#	if ___cplusplus >= 201103L
	KERBAL_TEST_CHECK_STATIC(could_use_field<FooPrivateField>::value == false);
#	endif
...
{
	KERBAL_TEST_CHECK_STATIC(could_use_method<FooNoMethod>::value == false);
	KERBAL_TEST_CHECK_STATIC(could_use_method<FooHasMethod>::value == true);

#if KERBAL_COMPILER_ID == KERBAL_COMPILER_ID_GNU
	KERBAL_TEST_CHECK_STATIC(could_use_method<FooPrivateMethod>::value == false);
#elif KERBAL_COMPILER_ID == KERBAL_COMPILER_ID_CLANG
#	if ___cplusplus >= 201103L
	KERBAL_TEST_CHECK_STATIC(could_use_method<FooPrivateMethod>::value == false);
#	endif
