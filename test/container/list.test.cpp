...
KERBAL_TEMPLATE_TEST_CASE(test_list_range_copy_construct, "test list::list(first, last)")
{
	...
	for (std::size_t tcase = 0; tcase <= N::value; ++tcase) {
		kerbal::container::list<int, Allocator> l(
			...
			kerbal::container::cbegin(arr) + tcase
		);
		KERBAL_TEST_CHECK(
		...
		KERBAL_TEST_CHECK(
			kerbal::compare::sequence_equal_to(
				l.crbegin(),
				l.crend(),
				kerbal::container::crend(arr) - tcase,
				kerbal::container::crend(arr)
			)
		); // r
...
KERBAL_TEMPLATE_TEST_CASE(test_list_iter_swap, "test list::iter_swap")
{
	...
	for (std::size_t i = 0; i < kerbal::container::size(c); ++i) {
		...
		kerbal::algorithm::iter_swap(
			kerbal::container::nth(ls, c[i].first),
			kerbal::container::nth(ls, c[i].second)
		);
...
KERBAL_TEST_CASE(test_list_reverse, "test list::reverse")
{
	...
	for (std::size_t tcase = 0; tcase < kerbal::container::size(range); ++tcase) {
		kerbal::container::list<int> l(
			kerbal::container::cbegin(l0),
			kerbal::container::cend(l0)
		);
		std::list<int> ls(
			kerbal::container::cbegin(l0),
			kerbal::container::cend(l0)
		...
		kerbal::algorithm::reverse(
			kerbal::container::nth(ls, b),
			kerbal::container::nth(ls, e)
		);
	...
	for (std::size_t pos = 0; pos <= kerbal::container::size(a_into); ++pos) {
		for (std::size_t opos = 0; opos < kerbal::container::size(a_from); ++opos) {
			kerbal::container::list<int> l_into(
				kerbal::container::cbegin(a_into),
				kerbal::container::cend(a_into)
			);
			kerbal::container::list<int> l_from(
				kerbal::container::cbegin(a_from),
				kerbal::container::cend(a_from)
			);
			...
			s_into.splice(
				kerbal::container::nth(s_into, pos),
				s_from,
				kerbal::container::nth(s_from, opos)
			);
...
KERBAL_TEST_CASE(test_list_splice_all, "test list::splice(list&)")
{
	...
	for (std::size_t pos = 0; pos <= kerbal::container::size(a0); ++pos) {
		kerbal::container::list<int> sl0(
			kerbal::container::cbegin(a0),
			kerbal::container::cend(a0)
		);
		kerbal::container::list<int> sl1(
			kerbal::container::cbegin(a1),
			kerbal::container::cend(a1)
		);
...
KERBAL_TEST_CASE(test_list_merge_is_stable, "test list::merge is_stable")
{
	...
	for (std::size_t tcase = 0; tcase < kerbal::container::size(size); ++tcase) {
		...
		Compare cmp;
		kerbal::algorithm::merge_sort(
			vp.begin(), vp.end(),
			test_list_merge_is_stable_sort_helper<int, Compare>(cmp)
		);
...
KERBAL_TEMPLATE_TEST_CASE(test_list_sort, "test list::sort")
{
	...
	for (std::size_t tcase = 0; tcase < kerbal::container::size(size_group); ++tcase) {
		...
		{ // sort partial range
			...
			KERBAL_TEST_CHECK(
				kerbal::compare::sequence_equal_to(
					ki::make_reverse_iterator(l.nth(500)),
					ki::make_reverse_iterator(l.begin()),
					ki::make_reverse_iterator(nth_500),
					ki::make_reverse_iterator(v.begin())
				)
			); // r
		}
...
KERBAL_TEMPLATE_TEST_CASE(test_list_radix_sort_is_stable, "test list::radix_sort is stable")
{
	...
	for (std::size_t tcase = 0; tcase < kerbal::container::size(size_group); ++tcase) {
...

		{
			...
			kerbal::algorithm::merge_sort(
				vp.begin(), vp.end(),
				list_radix_sort_is_stable_sort_helper<T, Order>(Order())
			);
